...
library(shinythemes)
# setwd("C:/Users/ykm25/Desktop/공모전/bigcontest2021/shiny2")
...
ui = navbarPage("제주시 행정동별 음식물 배출량의 이상치 탐지",
           tabPanel("Graphic",fluidPage(theme = shinytheme("flatly")),
                    tags$head(
                      ...
                      # tags$style(".tab-content {overflow: visible;}")),
                    pageWithSidebar(
                      headerPanel('
                                  Apply filters'),
...
server <- function(input, output) {
  ...
  #   if(is.null(anoms$timestamp) == T |anoms$timestamp[length(anoms$timestamp)] %>% as.Date()!=input$Date) {"이상치가 아닙니다."
  #     }else{paste0(input$Date, "의 음식물 배출량은 이상치입니다.")}
  # })
  
  output$anom_date = renderText({
    if( res()$anoms$timestamp[length(res()$anoms$timestamp)] %>% as.Date() ==input$Date) {"이상치가 탐지되었습니다."
    }else{"이상치가 아닙니다."}
  })
